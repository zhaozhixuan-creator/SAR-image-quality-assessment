...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        up = [
            ResizeConvBlock(
                up_in_channels=up_in_channels,
                out_channels=outer_channels,
                kernel_size=kernel_size,
                stride=1,
                padding=1,
                up_bias=up_bias,
                up_conv_cfg=up_conv_cfg,
                up_norm_cfg=up_norm_cfg,
                up_act_cfg=up_act_cfg,
                resize_cfg='bilinear',
                order=('act', 'conv', 'norm'))
        ]
...
class ResizeConvBlock(nn.Module):
    """Construct a Resize + Conv submodule to substitute the original deconv submodule, with the following.

    structure: resize + convolution.

    Args:
        outer_channels (int): Number of channels at the outer conv layer.
        inner_channels (int): Number of channels at the inner conv layer.
        resize_cfg (str): Config dict to resize. Default: 'linear'.
    ...
    def forward(self, x):
        ...
        return self.model(
                            F.interpolate(
                                            x, 
                                            size=(x.shape[2]*2+1, x.shape[3]*2+1), 
                                            mode=self.resize_cfg, 
                                            align_corners=True
                                          ))
